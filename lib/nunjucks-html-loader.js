...
function buildSiteMap(context) {
  ...
  templates.forEach(path => {
    const pathParts = path.replace('/pages/', '/').split('/').slice(1);
    ...
    pathParts.forEach((part, index) => {
      if (part.includes('.md')) {
        ...
        const page = {
          title,
          sortOrder,
          url: path.replace('/pages/', '/').replace('/index.md', '').replace('.md', '') || '/'
        };

        ref.push(page);

        if (key !== 'index' && pathParts.length === 1) {
          findNextRef(ref, '', page);
        }
      } else {
        findNextRef(ref, part);
      }
    });
  });
...
function findNextRef(ref, part, page) {
...
function getPageInfo(context) {
  ...
  let path = context.resourcePath.replace(context.rootContext, '').replace('/pages/', '/');
  ...
  pathParts.forEach((part, index) => {
    if (pageRef) {
      part = `/${part.replace('index.md', '').replace('.md')}`;
      pathRef += part;
